...
namespace App\Addons\TestNewAddon\Requests;
 ...
 * TestNewAddon 请求验证类
 */
class TestNewAddonRequest extends FormRequest
{
    /**
     * 确定用户是否有权限进行此请求
     */
    ...
    public function rules(): array
    {
        ...
        if ($this->isMethod('POST')) {
            // 创建时的特殊规则
            $rules['name'] .= '|unique:' . config('addons.TestNewAddon.database.table_prefix', 'TestNewAddon_') . 'data,name';
        } elseif ($this->isMethod('PUT') || $this->isMethod('PATCH')) {
            // 更新时的特殊规则
            $id = $this->route('id');
            $rules['name'] .= '|unique:' . config('addons.TestNewAddon.database.table_prefix', 'TestNewAddon_') . 'data,name,' . $id;
        }
